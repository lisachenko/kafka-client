...
/**
 * ReadShareGroupState, version 0: reads the state of share partitions (ApiKey 84, Kafka 4.1, KIP-932)
 *
 * <pre>
 ...
 * This one carries the whole state of a share partition - its state epoch, its start offset and the delivery state of its in-flight records - which a partition leader reads to load a share partition; a leader epoch above the recorded one is written back. The api is `"listeners": ["broker"]` and unstable in Kafka 3.9; it is stable from
 ...
 */
class ReadShareGroupStateRequest extends AbstractRequest
{
    /**
     * @inheritdoc
     */
    public const int API_KEY = ApiKeys::READ_SHARE_GROUP_STATE;
    ...
    /**
     * @param string                                $groupId       Id of the share group whose state is read
