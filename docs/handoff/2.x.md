...
ended in a gated milestone commit, and the whole branch was merged into `main` as one pull request. This file is the release record of the
line; the plan it was built from is kept below, under "The original plan", exactly as `docs/handoff/1.x.md` was
written.

The grammar is `docs/protocol/2.8.md` on the `2.x` branch (continued on `main` as
[`docs/protocol/4.3.md`](../protocol/4.3.md) by the 3.x line), the machine-readable frames are in
[`docs/protocol/vectors`](../protocol/vectors), and what a client cannot read out of the grammar is in that
document's "Broker quirks and observations" section. The line above this one (Kafka 3.x) started from `main` where the owner created the protected branch `2.x`, as
`1.x` was branched off at the end of the 1.x line; its record is [`docs/handoff/3.x.md`](3.x.md), and the plan of the
line above that one (4.x) is [`docs/handoff/main.md`](main.md).
...
of the protocol document to `docs/protocol/3.9.md`, the api keys **43-64** and the error codes **72-104** with one
...
[`docs/protocol/4.3.md`](../protocol/4.3.md) on this line). The finished 1.x tree was branched off as **`1.x`** at
the merge of PR #109, PR #110 wired the branch into the cascade and CI workflows and PR #111 cascaded
...
above every one of them, and the "API keys" section of `docs/protocol/3.9.md` is this answer.
...
Two things are deliberately **not** chronological: the **foundation** declares the api keys 43-64 and
