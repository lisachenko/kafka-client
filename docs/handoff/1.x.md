...
document's "Broker quirks and observations" section. The record of the **next** line is
[`docs/handoff/2.x.md`](2.x.md).
...
to `docs/protocol/3.9.md`, the api keys **34-42** and the error codes **56-71** with one exception class each.
...
questions — is kept below the release notes of that line in [`docs/handoff/2.x.md`](2.x.md).
...
`main` is the **1.x line**. It was identical to `0.11.x` at the branch point (the merge of PR #87) and
...
Run on 2026-09-10 with the `0.11.x` tree at its final commit against the 1.1.1 container above, all four listeners:
...
- **T1 broker, surface, docs**: `docker/kafka-1.1.1/`, the protocol document renamed to `docs/protocol/3.9.md`
