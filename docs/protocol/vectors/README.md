...
========================================
The 4.x line has captured **252** vectors of its own so far, on the `kafka-4-3-1` node — the 40 frames of Kafka 4.2
...
in `share-fetch.json` and `share-acknowledge.json`. And the 10 frames of Kafka 4.2 on DescribeShareGroupOffsets v1 and
WriteTxnMarkers v2. And the 40 frames of the
...
hex dumps. There are **1387** of them in **74** files: **40** are the offset and share-group apis of Kafka 4.2 (OffsetCommit
and OffsetFetch v10, ShareFetch and ShareAcknowledge v2), captured on the `kafka-4-3-1` node - **10** are the two
versions Kafka 4.2 added to the admin and
transaction apis, captured on the `kafka-4-3-1` node - the DescribeShareGroupOffsets v1 pairs of the share-partition lag
of KIP-1226 in `describe-share-group-offsets.json`, measured with real share traffic, and the three WriteTxnMarkers v2
pairs of the transaction version of KIP-1228 in `write-txn-markers.json` - **40** are the share-group apis of Kafka 4.1 (KIP-932), captured on the
