...
hex dumps. There are **378** of them in **36** files: **60** were captured on the `kafka-2-8-2` container of the **2.x**
line - the request and the answer of every version Kafka **2.0** added to the admin, the transaction and the
delegation-token apis (34 frames) and to the ten group apis (20 frames), all of them KIP-219 bumps, plus the
6 frames of what Kafka **2.1** added to OffsetCommit and OffsetFetch - and of the
