...
========================================
The 3.x line has captured **273** vectors of its own so far, on the `kafka-3-9-2` KRaft node — the 49 frames of Kafka 3.0:
...
the same questions with the 22 and the 35 — and the 58 frames of Kafka 3.7, the 12 of DescribeCluster v1 (the
endpoint type of KIP-919, with the 114 of a controller asked on a broker listener and the 115 of a type the api
does not define) in `describe-cluster.json` and the 26 of the three client-metrics apis of KIP-714 in the three
new files `get-telemetry-subscriptions.json`, `push-telemetry.json` and `list-client-metrics-resources.json`,
which are wire only, and the 20 of OffsetFetch v9 of KIP-848 in `offset-fetch.json`, with the 113 of a stale
...
hex dumps. There are **942** of them in **57** files: **351** were captured on the `kafka-2-8-2` container of the
