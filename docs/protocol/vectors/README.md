...
========================================
The 3.x line has captured **314** vectors of its own so far, on the `kafka-3-9-2` KRaft node — the 49 frames of Kafka 3.0:
...
the 28 frames of Kafka 3.8, the 6 of Produce v11 in `produce.json`: the accepted pair, the pair of a
...
because no FindCoordinator ever answers the 120 the version exists for) — next
to the
...
hex dumps. There are **983** of them in **57** files: **351** were captured on the `kafka-2-8-2` container of the
