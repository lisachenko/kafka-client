...
========================================
The 3.x line has captured **292** vectors of its own so far, on the `kafka-3-9-2` KRaft node — the 49 frames of Kafka 3.0:
...
the 6 frames of Kafka 3.8, all of them Produce v11 in `produce.json`: the accepted pair, the pair of a
transactional batch for a partition the coordinator has not verified, which is the **120** `TransactionAbortable`
of KIP-890, and the pair of the very same request at version 10, which is answered the **48** instead — next
to the
...
hex dumps. There are **961** of them in **57** files: **351** were captured on the `kafka-2-8-2` container of the
