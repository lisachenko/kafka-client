...
========================================
The 4.x line has captured **242** vectors of its own so far, on the `kafka-4-3-1` node — the 40 frames of Kafka 4.2
...
in `share-fetch.json` and `share-acknowledge.json`. And the 40 frames of the
...
hex dumps. There are **1377** of them in **74** files: **40** are the offset and share-group apis of Kafka 4.2 (OffsetCommit
and OffsetFetch v10, ShareFetch and ShareAcknowledge v2), captured on the `kafka-4-3-1` node - **40** are the
share-group apis of Kafka 4.1 (KIP-932), captured on the
