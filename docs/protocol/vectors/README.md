...
========================================
The 3.x line has captured **253** vectors of its own so far, on the `kafka-3-9-2` KRaft node — the 49 frames of Kafka 3.0:
...
the same questions with the 22 and the 35 — and the 38 frames of Kafka 3.7, the 12 of DescribeCluster v1 (the
...
which are wire only — next to the
...
hex dumps. There are **922** of them in **57** files: **351** were captured on the `kafka-2-8-2` container of the
