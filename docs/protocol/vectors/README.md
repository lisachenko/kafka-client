...
========================================
The 3.x line has captured **310** vectors of its own so far, on the `kafka-3-9-2` KRaft node — the 49 frames of Kafka 3.0:
...
the 24 frames of Kafka 3.8, the five version bumps of KIP-890 part 2, none of which declares a field: the 4 of
InitProducerId v5 in `init-producer-id.json` (with the 90 of an epoch that is neither the current nor the last
...
hex dumps. There are **979** of them in **57** files: **351** were captured on the `kafka-2-8-2` container of the
