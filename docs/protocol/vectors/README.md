...
========================================
The 3.x line has captured **228** vectors of its own so far, on the `kafka-3-9-2` KRaft node — the 49 frames of Kafka 3.0:
...
the same questions with the 22 and the 35 — and the 13 frames of Kafka 3.7, the leader discovery of KIP-951: the 5
of Produce v10 in `produce.json`, one of them the constructed answer that names the current leader of a refused
partition, and the 8 of Fetch v16 in `fetch.json`, among them the follower fetch the node really answers with the
`current_leader` and the `node_endpoints` — next to the
...
hex dumps. There are **897** of them in **54** files: **351** were captured on the `kafka-2-8-2` container of the
