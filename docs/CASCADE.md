...
and one tag per minor - `2.0.1` … `2.8.2`; its record is [`docs/handoff/main.md`](handoff/main.md) until the 3.x
line renames it to `docs/handoff/2.x.md`), and `main` is where the **3.x line** starts, from the plan in
[`docs/handoff/3.x.md`](handoff/3.x.md). `2.x` is a **protected** branch: no direct pushes, changes arrive by PR
and leave by the cascade.
...
| 2.x | main | **wired** — `2.x` was created by the owner at `bc5dec5`, the merge of PR #141 that completed the 2.x line (epic #112), so `2.x` and `main` are identical at the branch point; this row's PR adds `2.x` to the cascade workflow. The nine milestone commits of the line (`2afcb2f` 2.0, `72d1bb3` 2.1, `8c9fae9` 2.2, `d2b20c6` 2.3, `3e21b50` 2.4, `f85c7df` 2.5, `9978b33` 2.6, `c61f52c` 2.7, `53d91b2` 2.8) are in both histories and carry the tags `2.0.1` … `2.8.2`. `main` is now where the **3.x line** starts: its plan is [`docs/handoff/3.x.md`](handoff/3.x.md), its broker is Kafka **3.9.2** (the last 3.x release and the last one with ZooKeeper), its first ticket renames `docs/handoff/main.md` to `docs/handoff/2.x.md` and `docs/handoff/3.x.md` to `docs/handoff/main.md`, as the 2.x line did with `2.0.x.md`. Every fix that lands on `2.x` arrives through a cascade PR |
