...
is merged upwards into the next one:
    ...
    0.8.x (Kafka 0.8.2.2) → 0.9.x (Kafka 0.9.0.1) → 0.10.x (Kafka 0.10.2.2) → 0.11.x (Kafka 0.11.0.3) → main (Kafka 1.1.1, complete)
...
2. **Trigger.** Every push to `0.8.x`, `0.9.x`, `0.10.x` or `0.11.x` makes `.github/workflows/cascade.yml`
   ...
   has to be added to the workflow's trigger list and to the chain above, as `0.11.x` was.
