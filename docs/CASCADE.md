...
| 0.9.x | 0.10.x | **pending** — `0.9.x` is complete (api keys 0-16, Produce/Fetch v1, OffsetCommit v2, ControlledShutdown v1, the group membership and group introspection apis, the consumer protocol with both assignors, SSL). The pre-schema 0.10 code currently on `0.10.x` (commits `5b6dd06..77993bc`: Metadata v1/v2, Fetch v2/v3, Offsets v1, JoinGroup v1, OffsetFetch v2, ApiVersions, SaslHandshake, message format with timestamps) is dropped by rule 5 and re-implemented as the 0.10.x tickets — see `docs/handoff/0.10.x.md` |
