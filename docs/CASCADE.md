...
8. **A fix on a frozen major line** (`1.x`, `2.x`, `3.x`) is made on a `t<n>-<slug>` branch off that line, gated against
   that line's broker (`docker compose up` on the line's own checkout builds its image, `docker/kafka-1.1.1`,
   `docker/kafka-2.8.2` or `docker/kafka-3.9.2`), merged into the line by PR with a merge commit, and then cascades upwards through every
   line above it - a fix on `1.x` arrives on `main` through `2.x` and `3.x`. A fix never adds a wire version to a frozen
