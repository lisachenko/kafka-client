...
| 0.10.x | main | **not merged yet.** `0.10.x` is complete (Kafka 0.10.2.2: ApiVersions, message format v1 with LZ4, Metadata v1/v2, Produce v2, Fetch v2/v3, Offsets v1, OffsetFetch v2, JoinGroup v1, CreateTopics/DeleteTopics, SASL/PLAIN, error codes to 44). The automatic PR #48 conflicts in nearly every file and is to be closed in favour of a `cascade/0.10.x-into-main` branch; the pre-schema code of `main` (the 14 requests with `$header = null`, the untyped `RecordBatch`, the `KafkaException` that stops at 35, the six remaining tests) is dropped by rule 5 and re-implemented on the 0.10 design as the tickets of the 0.11 line — see `docs/handoff/main.md` |
