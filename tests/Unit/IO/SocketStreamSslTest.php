 ...
 *
 * The certificate is the one the test broker presents on its SSL listener (`docker/kafka-4.3.1/ssl`), so the
 ...
 * @see docs/protocol/4.3.md, section "Transport security (SSL)"
 */
...
final class SocketStreamSslTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        if (!is_readable(self::certificateFile()) || !is_readable(self::keyFile())) {
            self::markTestSkipped('The test certificate of docker/kafka-4.3.1/ssl is not available');
        }
    }
    ...
    {
        return dirname(__DIR__, 3) . '/docker/kafka-4.3.1/ssl/broker.crt';
    }

    private static function keyFile(): string
    {
        return dirname(__DIR__, 3) . '/docker/kafka-4.3.1/ssl/broker.key';
    }
