...
| `findCoordinator()`                          | GroupCoordinator v5     | Retries the codes 15 and 14 while the coordinator warms up; version 1 also looks a **transactional id** up (`coordinator_type = 1`); one key travels as a one-element batch of the v4 of KIP-699 (Kafka 3.0), and `Client::getGroupCoordinators()` / `getTransactionCoordinators()` look several up at once; the v5 of KIP-890 (Kafka 3.8) added no field, only the promise of the error code 120 |
...
| 1 | Fetch | v0 … v17 | yes | v0 … v11, **v12** (session-less in `fetchPartitions()`, with an **incremental fetch session per broker** in the consumer) | v0 … v14, **v15** (the tagged `replica_state` of KIP-903, Kafka 3.5) and **v16** (the tagged `node_endpoints` of the answer, KIP-951, Kafka 3.7, which the node writes for the 6 of a follower fetch; every topic named by its **topic id**, KIP-516, Kafka 3.1; session-less in `fetchPartitions()`, with an incremental fetch session per broker in the consumer; `FetchRequestV12` keeps the frame that names its topics, `FetchRequestV13` to `FetchRequestV16` the versions below), and **v17** (the tagged `replica_directory_id` of KIP-853, Kafka 3.9, which a consumer leaves off the wire; `FetchRequest::getReplicaDirectoryId()`) |
| 2 | Offsets (ListOffsets) | v0 … v9 | yes | v0 … v5, **v6** | v0 … v7, **v8** (the local log start offset `-4` of KIP-405, Kafka 3.5, `AdminClient::listEarliestLocalOffsets()`; `OffsetsRequestV7` keeps the version of the max timestamp `-3` of KIP-734, Kafka 3.0), and **v9** (the last tiered offset `-5` of KIP-1005, Kafka 3.9, `AdminClient::listLatestTieredOffsets()`; `OffsetsRequestV8` keeps the version below) |
...
| 10 | GroupCoordinator (FindCoordinator) | v0 … v6 | yes | v0 … v2, **v3** | v0 … v3, **v4** (the `coordinator_keys` batch of KIP-699, Kafka 3.0; `GroupCoordinatorRequestV3` keeps the single key), **v5** (the same frame with the promise of the code 120 of KIP-890, Kafka 3.8, `GroupCoordinatorRequestV4`/`ResponseV4` keep the version below), and **v6** (the coordinator type 2 of KIP-932, Kafka 3.9, the version this client sends; `GroupCoordinatorRequestV5`/`ResponseV5` keep the version below) |
