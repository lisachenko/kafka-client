...
reached is **Kafka 4.0**: the client speaks every version Kafka 4.0 added to the apis it implements — the
transaction protocol v2 of KIP-890 part 2 among them — on top of the **KIP-848 consumer protocol** of the 3.x line,
and the versions Kafka 4.0 removed (KIP-896) are never sent to a 4.x node. The version of every api the client
...
line** (its record is [docs/handoff/3.x.md](docs/handoff/3.x.md)). Today the client speaks **Kafka 4.0** —
everything Kafka 3.0 to 3.9 and 4.0 added, and the KIP-848 consumer protocol — against a **4.3.1** node, and the
table below lists the version of every api the client sends; what Kafka 4.1 to 4.3 add arrives one gated milestone
...
on this line** until its milestone lands. The share-group apis 76–79 and 90–92, the share-group state apis
83–87 and the streams-group apis 88 and 89 are in the "leaves out" table below until their milestones. The
`2.x` column is where the 3.x line started.

| Api key | API | Versions served by 4.3.1 | Client-facing | `2.x` | `main` (4.x, towards Kafka 4.3.1) |
|---|---|---|---|---|---|
| 0 | Produce | v3 … v13 (v0 … v2 listed, refused) | yes | v0 … v8, **v9** (**v2** for `message.format.version` below 0.11.0) | v0 … v11, **v12** (Kafka 4.0, KIP-890 part 2: the v11 frame, sent outside transactions and inside a transaction of the protocol v2 through `Client::produceVersion()`; `ProduceRequestV11` keeps the version a transaction of the protocol v1 is capped at, and the **120** `TransactionAbortable` of KIP-890, Kafka 3.8, it is refused with for a partition the coordinator has not verified; `ProduceRequestV10`/`ProduceResponseV10` keep the leader discovery of KIP-951; **v2** for `message.format.version` below 0.11.0, refused client-side against a 4.x node) — *not yet implemented on this line*: v13 (4.1) |
...
| 45 | AlterPartitionReassignments | v0, v1 | controller | **v0** (Kafka 2.4) | **v0** (Kafka 2.4) |
...
| 66 | ListTransactions | v0, v1, v2 | yes | – | **v0, v1** (`AdminClient::listTransactions()`, Kafka 3.0; the duration filter of KIP-994, Kafka 3.8; `ListTransactionsRequestV0` keeps the version below) |
...
| 74 | ListClientMetricsResources | v0, v1 | yes | – | **v0, wire only** (Kafka 3.7, KIP-714) — classes and vectors, no client method |
