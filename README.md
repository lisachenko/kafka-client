...
reached is **Kafka 4.1**: the client speaks every version Kafka 4.0 and 4.1 added to the apis it implements — the
transaction protocol v2 of KIP-890 part 2, Produce by topic id and the share-group wire of KIP-932 among them — on
...
| `addRaftVoter()` / `removeRaftVoter()`       | AddRaftVoter v0 / RemoveRaftVoter v0 | Adds or removes a voter of the metadata quorum by its key (the id and the directory id) on a `kraft.version` 1 cluster (KIP-853, Kafka 3.9; the admin api of Kafka 4.0): a voter id the quorum has is the 126, a key it does not hold the 127, and a voter that is not reachable is the 7 and never added |
...
line** (its record is [docs/handoff/3.x.md](docs/handoff/3.x.md)). Today the client speaks **Kafka 4.1** —
everything Kafka 3.0 to 3.9, 4.0 and 4.1 added, and the KIP-848 consumer protocol — against a **4.3.1** node, and the
table below lists the version of every api the client sends; what Kafka 4.2 and 4.3 add arrives one gated milestone
at a time (the plan is [docs/handoff/main.md](docs/handoff/main.md)). The frozen protocol snapshots of the lines below
...
| 2 | Offsets (ListOffsets) | v1 … v11 | yes | v0 … v5, **v6** | v0 … v9, **v10** (Kafka 4.0, the `timeout_ms` of KIP-1075; `OffsetsRequestV9` keeps the last tiered offset `-5` of KIP-1005, Kafka 3.9, `AdminClient::listLatestTieredOffsets()`, `OffsetsRequestV8` the local log start offset `-4` of KIP-405, Kafka 3.5, `AdminClient::listEarliestLocalOffsets()`) — *not yet implemented on this line*: v11 (4.2) |
...
| 8 | OffsetCommit | v2 … v10 | yes | v0 … v7, **v8** (**v0** for `offsets.storage = zookeeper`) | v0 … v8, **v9** (Kafka 3.6, the v8 frame that may carry the 69 of an unknown group and the 113 of a KIP-848 member epoch; `OffsetCommitRequestV8` keeps the flexible v8) |
| 9 | OffsetFetch | v1 … v10 | yes | v0 … v6, **v7** (**v0** for `offsets.storage = zookeeper`) | v0 … v7, **v8** (several groups in one request, Kafka 3.0), **v9** (the member id and member epoch of KIP-848 per group entry, Kafka 3.7 — the version this client sends; `OffsetFetchRequestV8` keeps the batch of Kafka 3.0) |
...
| 27 | WriteTxnMarkers | v1, v2 | broker→broker | v0, **v1** (classes and vectors; a broker→broker api, probed only) | v0, **v1** (classes and vectors; a broker→broker api, probed only) |
...
| 78 | ShareFetch | v1, v2 | yes | – | **v1** (Kafka 4.1: `Client::shareFetch()`, the share session and the acquired records with their delivery count) — *not yet implemented on this line*: v2 (4.2) |
| 79 | ShareAcknowledge | v1, v2 | yes | – | **v1** (Kafka 4.1: `Client::shareAcknowledge()`, the close of the share session with the epoch -1) — *not yet implemented on this line*: v2 (4.2) |
| 80 | AddRaftVoter | v0, v1 | yes (a raft api the client listener serves, forwarded to the controller) | – | **v0** (Kafka 3.9, KIP-853; `AdminClient::addRaftVoter()`, the admin api of Kafka 4.0 on the `kraft.version` 1 node: the 126 of a voter id the quorum has, the 104, the 42, and the 7 of an unreachable voter, which is never added) — *not yet implemented on this line*: v1 (4.2) |
...
| 83–87 | The share-group state apis (KIP-932) | v0 (85 and 87: v0, v1) | yes (the share coordinator's own apis) | – | **v0, wire only** (Kafka 4.1): classes and vectors, no client method — *not yet implemented on this line*: v1 of 85 and 87 (4.2) |
...
| 90 | DescribeShareGroupOffsets | v0, v1 | yes | – | **v0, wire classes** (Kafka 4.1, KIP-932); the admin method comes with the share consumer — *not yet implemented on this line*: v1 (4.2) |
